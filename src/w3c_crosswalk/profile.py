"""Project ACDC credentials into the repository's W3C crosswalk profile.
...
class CrosswalkProfileError(ValueError):
    """Raised when an ACDC credential cannot be projected into the crosswalk profile."""
...
def schema_type(acdc: dict[str, Any]) -> str:
    ...
    if schema not in SUPPORTED_SCHEMA_TYPES:
        raise CrosswalkProfileError(f"unsupported schema SAID: {schema}")
...
def build_crosswalk_metadata(acdc: dict[str, Any]) -> dict[str, Any]:
    """Build the crosswalk provenance block embedded in the W3C VC."""
    edges = require_mapping("edges", acdc.get("e", {}))
    return {
        "profile": CROSSWALK_PROFILE,

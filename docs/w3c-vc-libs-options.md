...
`isomer` is not relying only on its own hand-rolled VC-JWT verifier.
...
## Current `isomer` verifier legitimacy

Our verifier in [`src/vc_isomer/verifier.py`](src/vc_isomer/verifier.py)
...
- Weaknesses:
  ...
  - not a clean generic verifier for our isomer profile
- Recommendation:
...
- Our current verifier is necessary because it knows about isomer-specific
  lineage, ACDC provenance, and projected KERI status.
- An external verifier is still valuable because it tells us whether our
  VC-JWTs are understandable outside the isomer codebase.
